...
final class DecodeHandlerForBarcode extends Handler {
  ...
  DecodeHandlerForBarcode(BarcodeActivity activity, Hashtable<DecodeHintType, Object> hints) {
...
//     PlanarYUVLuminanceSource source = CameraManager.get().buildLuminanceSource(data, height, width);
    ...
    if (rawResult != null) {
      ...
      Message message = Message.obtain(activity.getHandler(), CaptureActivity.decode_succeeded, rawResult);
      ...
      Message message = Message.obtain(activity.getHandler(), CaptureActivity.decode_failed);
      message.sendToTarget();
    }
  }
